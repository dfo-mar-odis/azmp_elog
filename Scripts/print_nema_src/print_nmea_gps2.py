...
def find_characters(nmea: str, target: str):
    if not isinstance(nmea, str) or not isinstance(target, str):
        raise TypeError("Both nmea and target must be strings.")

    pos = nmea.find(target)  # returns -1 if not found
    return pos
...
def get_stream(udp_ip, udp_port, return_queue):
    ...
    # $INGGA,160316.16 ,4124.851593,N,06039.877057,W,2,12,0.7,3.78,M,-24.67,M,7.0,0073*60
    GGA = None
